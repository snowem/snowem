...
snw_ice_cb_new_selected_pair(agent_t *agent, uint32_t stream_id,
   ...
   //TODO: set timeout to call dtls_retry
   return;
}
...
send_rtcp_pkt_internal(snw_ice_session_t *session, int video, int encrypted, char *buf, int len) {
   ...
   //TODO: check cdone equal to num of stream
...
snw_ice_broadcast_rtp_pkg(snw_ice_session_t *session, 
   ...
   for (int i=0; i<session->subscribe->idx; i++) {
      ...
      if (session->subscribe->players[i] != 0) {
         ...
         streamid = session->subscribe->players[i];
         ...
         s = (snw_ice_session_t*)snw_ice_session_search(ice_ctx, streamid);
         if (s) {
            ...
            { // handle pkg out
               ...
               remote_ssrc = (video ? s->audio_stream->local_video_ssrc :
...
void ice_data_recv_cb(agent_t *agent, uint32_t stream_id,
   ...
   if (pt == DTLS_PT) {
      DEBUG(log, "dtls incoming msg, %p", component);
   ...
   if (!component->dtls 
      ...
      if (pt == RTP_PT) {
         ...
         DEBUG(log, "sctp incoming msg, %p", component);
      }
   }
...
snw_ice_create_media_stream(snw_ice_session_t *session, int video) {
   ...
   rtp->dtls = dtls_create(session->ice_ctx, rtp, stream->dtls_type);
   if (rtp->dtls == 0) {
      DEBUG(log, "failed to create dtls context");
      return -4;
   }
...
snw_ice_connect_msg(snw_ice_context_t *ice_ctx, void *data, int len, uint32_t flowid) {
   ...
   DEBUG(log,"init new session, streamid=%u, stream_type=%u, flowid=%u",
         streamid, stream_type, session->flowid);
...
snw_ice_session_free(snw_ice_context_t *ice_ctx, snw_ice_session_t *session) {
   ...
   if (session->publishid != 0) {
      snw_subscribe_remove_subscriber(ice_ctx, session->publishid,
        session->streamid);
   }
...
int ice_setup_remote_credentials(snw_ice_session_t *session, snw_ice_stream_t *stream, snw_ice_component_t *component) {
   ...
   TAILQ_FOREACH(c,&component->remote_candidates,list) {
      ...
      if (address_is_private(&(c->addr)) ) {
         ...
         address_to_string(&(c->addr), (char *)&address);
         // TODO: removing private ips causes failure of ICE process
   ...
   if (ret >= 7) {
      ...
      if (c != 0) {
         ...
         if (!strcasecmp(transport, "udp")) {
            ...
            // TODO: support other transport, see secion-4.5 in rfc6544
            candidate_free(c);
            return -4;
         }
...
snw_ice_publish_msg(snw_ice_context_t *ice_ctx, void *data, int len, uint32_t flowid) {
   ...
   DEBUG(log, "stream is publishing, flowid=%u, streamid=%u", flowid, streamid);
...
snw_ice_play_msg(snw_ice_context_t *ice_ctx, void *data, int len, uint32_t flowid) {
   ...
   if (streamid == (uint32_t)-1 || publishid == (uint32_t)-1) {
     ERROR(log, "no streamid or publiserid, flowid=%u, streamid=%u, publishid=%u",
         flowid, streamid, publishid);
     return;
   }
   ...
   snw_subscribe_add_subscriber(ice_ctx, publishid, streamid);
